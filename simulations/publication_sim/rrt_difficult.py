...
from stl_tool.stl                     import GOp, FOp, TasksOptimizer, BoxBound, ContinuousLinearSystem, ISSDeputy,SingleIntegrator3d, Predicate
...
box_predicate_1 : Predicate =  BoxBound(dims = [0,1,2], size = 70, center = interest_point_1_center)
visit_time1                 = 100.

interest_point_2_center     = np.array([-100., -100., 0.])
box_predicate_2 : Predicate =  BoxBound(dims = [0,1,2], size = 70, center = interest_point_2_center)
visit_time2                 = 250.

interest_point_4_center     = np.array([0., 0., 100.])
box_predicate_4 : Predicate =  BoxBound(dims = [0,1,2], size = 70, center = interest_point_4_center)
visit_time4                 = 380.

interest_point_3_center     = np.array([100., 100., 0.])
box_predicate_3 : Predicate =  BoxBound(dims = [0,1,2], size = 70, center = interest_point_3_center)
visit_time3                 = 480.

visit_period                = 40


formula        = (GOp(0.1,500) >> (FOp(0,300) >> box_predicate_1)) &  (GOp(50,100) >> box_predicate_2) & (FOp(300,340) >> box_predicate_4)
...
##########################################################
x_0       = box_predicate_1.polytope.sample_random() # sample point from the predicate
